class PathFilter:
    ...
    def is_suspicious_path(path: str) -> bool:
        ...
        for skip in PathFilter.HARD_SKIPS:
            ...
            if (
                f"/{skip_lower}/" in path_lower
                or f"{skip_lower}/" in path_lower
                or path_lower.startswith(skip_lower)
            ):
                return True
        return False
    ...
    def is_likely_cache_file(filename: str, extension: str) -> bool:
        ...
        if ext_lower in PathFilter.BINARY_EXTENSIONS:
            return True
        if not ext_lower:
            if len(filename_lower) > 20 and all(
                c in "0123456789abcdef" for c in filename_lower
            ):
                return True
    ...
    def should_include(path: str, filename: str, extension: str) -> bool:
        ...
        return not (
            PathFilter.is_suspicious_path(path)
            or PathFilter.is_likely_cache_file(filename, extension)
        )
