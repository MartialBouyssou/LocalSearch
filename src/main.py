...
def _get_path_completions(
    partial_path: str, search_context: SearchContext
) -> list[str]:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="LocalSearch", description="Local file search engine (SQLite)."
    )
    ...
    p.add_argument(
        "--search", type=str, default=None, help="Run a one-shot search query."
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Commit every N files during reindex.",
    )
    p.add_argument(
        "--max-text-bytes",
        type=int,
        default=None,
        help="Max bytes to read for text-like files.",
    )
    p.add_argument(
        "--sample-bytes",
        type=int,
        default=None,
        help="Sample bytes for large text files (CSV/TSV).",
    )
    p.add_argument(
        "--no-lazy-upgrade",
        action="store_true",
        help="Disable lazy upgrade of partial docs.",
    )
    p.add_argument(
        "--include-soft-skips",
        action="store_true",
        help="Include .venv, node_modules, .git, etc.",
    )
    p.add_argument(
        "--no-watch",
        action="store_true",
        help="Disable file watcher in interactive mode.",
    )
    p.add_argument(
        "--debounce",
        type=float,
        default=None,
        help="Debounce seconds for file watcher.",
    )
    p.add_argument(
        "--save-config",
        action="store_true",
...
def main() -> None:
    ...
    extractor = ContentExtractor(
        ...
        cfg=ExtractorConfig(
            max_text_bytes=cfg.max_text_bytes, sample_bytes=cfg.sample_bytes
        ),
    )
    ...
    if args.search is not None:
        ...
        try:
            results = engine.search(
                args.search, top_k=cfg.topk, lazy_upgrade=not cfg.no_lazy_upgrade
            )
        finally:
            engine.close()
        elapsed = time.time() - start
    ...
    if not need_index and args.search is None:
        ...
        try:
            if not cfg.no_watch:
                incremental = IncrementalIndexingService(
                    db_path=cfg.db, extractor=extractor
                )
