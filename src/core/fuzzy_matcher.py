...
class FuzzyMatcher:
    ...
    def is_fuzzy_match(query_term: str, candidate_term: str) -> Tuple[bool, float]:
        ...
        """
        if (
            len(query_term) < FuzzyMatcher.MIN_LENGTH
            or len(candidate_term) < FuzzyMatcher.MIN_LENGTH
        ):
    ...
    def find_fuzzy_matches(
        query_terms: List[str], candidate_terms: List[str], max_results: int = 50
    ) -> List[Tuple[str, float]]:
        """Find fuzzy matches for query terms against a candidate list."""
        ...
        for query_term in query_terms:
            ...
            for candidate_term in candidate_terms:
                if candidate_term in matches:
                    continue
                is_match, confidence = FuzzyMatcher.is_fuzzy_match(
                    query_term, candidate_term
                )
                if is_match and confidence > 0:
                    matches[candidate_term] = max(
                        matches.get(candidate_term, 0), confidence
                    )
