...
class BM25Ranker:
    ...
    def __init__(self, index, k1: float = 1.5, b: float = 0.75):
        """
        Initialize BM25 ranker with parameters.
        
        Args:
            index: InvertedIndex instance for accessing term and document statistics.
            k1: Saturation parameter controlling term frequency impact (typical: 1.5).
            b: Length normalization parameter (typical: 0.75, range: 0-1).
        """
        self.index = index
        self.k1 = k1
        self.b = b
    ...
    def _compute_avg_doc_length(self) -> float:
        """
        Compute the average number of indexed bytes across all documents.
        
        Attempts to fetch from the index, defaults to 256,000 bytes if unavailable.
        
        Returns:
            Average indexed bytes per document.
        """
